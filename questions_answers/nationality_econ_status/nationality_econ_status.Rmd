...
selected_data <- zp_merged_data %>% select(HIGHESTCOMPLEDU, HIGHESTCOMPLEDUAGGII, AGE,
                          ...
                          CURROCCUPATIONISCO,CURROCCUPATIONISCOAGG,MAINLANGUAGEIIIAGG,MAINLANGUAGEIIIAGG,MAINLANGUAGEIIIAGG)
```
...
```{r}
main_languages <- list(
  "1" = "Deutsch (oder Schweizerdeutsch)",
  "2" = "Französisch (oder Patois Romand)",
  "3" = "Italienisch (oder Tessiner/Bündner-italienischer Dialekt)",
  "4" = "Rätoromanisch",
  "5" = "Serbisch/Kroatisch",
  "6" = "Albanisch",
  "7" = "Portugiesisch",
  "8" = "Spanisch",
  "9" = "Englisch",
...
selected_data$knows_official_language <- apply(
  ...
  1,
  function(row) any(row %in% c("Deutsch (oder Schweizerdeutsch)", "Französisch (oder Patois Romand)", "Italienisch (oder Tessiner/Bündner-italienischer Dialekt)", "Englisch"))
)
```

```{r}
foreigners <- selected_data %>% filter(NATIONALITYCAT == "Ausländer" &
                                         YEAR_QUIZ != 2020 &
                                         YEAR_QUIZ != 2021)
```
